...
public class Dungeon implements IDungeon {
  ...
  public Dungeon(int interConnectivity, String name, boolean wrapped,
    ...
    if (getAllCaves().size() < numberOfMonsters) {
      this.numberOfMonsters = getAllCaves().size();
    }
  ...
  private IRoom[][] constructDungeon(int rows, int columns,
    ...
    while (findShortestPath(start, end, dungeon) < MINIMUM_DISTANCE
            || dungeon[end[0]][end[1]].isTunnel()) {
