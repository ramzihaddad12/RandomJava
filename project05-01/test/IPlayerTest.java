...
public class IPlayerTest {
  ...
  /**
   * Tests the isInDanger method when no
   * pit nearby.
   */

  @Test
  public void testIsInDangerWhenNoPitNearby() {
    assertFalse(player.isInDanger());
  }

  /**
   * Tests the isInDanger method when pit
   * nearby.
   */

  @Test
  public void testIsInDangerWhenPitNearby() {
    int [] positionOfRoom = new int[] {2, 3};
    IRoom room = new Room(positionOfRoom);
    IRoom otherRoom1 = new Room(new int[] {2, 4});
    ...
    room.addTreasures(treasureList);
    room.addArrows(1);
    IPlayer newPlayer = new Player("Ben", room);

    assertTrue(newPlayer.isInDanger());
  }
  ...
  @Test
  public void testRemoveAllTreasures() {
    int [] positionOfRoom = new int[] {2, 3};
    IRoom room = new Room(positionOfRoom);
    IRoom otherRoom1 = new Room(new int[] {2, 4});
    ...
    room.addArrows(1);
    IPlayer newPlayer = new Player("Ben", room);
    newPlayer.pickUpTreasure();

    assertEquals(treasureList, newPlayer.getTreasures());

    newPlayer.removeAllTreasures();

    assertTrue(newPlayer.getTreasures().isEmpty());
  }
