...
public class Dungeon implements IDungeon {
  ...
  private boolean fellInPit;
  private boolean gotStolenFrom;
  ...
  public Dungeon(int interConnectivity, String name, boolean wrapped,
                 ...
                 int numberOfPits, Randomness randomness) {
    if (randomness == null) {
      throw new IllegalArgumentException("Randomness cannot be null");
    }
    ...
    if (numberOfMonsters > rows * columns || numberOfPits > (rows * columns - 2)) {
      throw new IllegalArgumentException("Number of monsters/pits greater than number of rooms");
    }
    ...
    if (this.player.getPosition().hasAMonster()) {
      isGameOver = true;
      didPlayerWin = false;
    }
  }
  ...
  private void addThief() {
    int xThief = 1;
    int yThief = 1;
    this.grid[xThief][yThief].addThief();
    return;
  }

  private void addPits(int numberOfPits) {

    int count = 0;


    while (count < numberOfPits) {
      int x = randomness.generateValueBetween(0, rows - 1);
      int y = randomness.generateValueBetween(0, columns - 1);

      if (!Arrays.equals(this.start, new int[] {x, y})
              && !Arrays.equals(this.end, new int[] {x, y})
              && !this.grid[x][y].hasPit()
              && !this.grid[x][y].hasAMonster()) {
        this.grid[x][y].addPit();
        count++;
      }
    }
  ...
  public void movePlayer(Direction direction) {
    ...
    if (roomPlayerIsIn.hasThief()) {
      this.gotStolenFrom = true;
      this.player.removeAllTreasures();
    }
  ...
  private int[] getArrowEndLocation(int distance, Direction shootingDirection, IRoom startRoom) {
    ...
    return new int[] {room.getPosition()[0], room.getPosition()[1]};
  }
  ...
  @Override
  public boolean didPlayerFallInPit() {
    return fellInPit;
  }

  @Override
  public boolean didPlayerGetRobbed() {
    return this.gotStolenFrom;
  }
